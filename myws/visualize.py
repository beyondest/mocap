...
def visualize_2d_pose(kps_output:np.ndarray, 
    ...
    for_draw = np.zeros((17,2),dtype=np.float32)
    for index,each_kpt in enumerate(kps_output[0]):
        if each_kpt[2] > conf:
             for_draw[index][0] = each_kpt[0]
             for_draw[index][1] = each_kpt[1] if not if_invert_y else -each_kpt[1]
    ax2.clear()
    plot_pose_2d(ax=ax2, 
                 tar=for_draw, 
                bones=bones, 
...
def plot_pose_3d(
    ...
    limb_color : list of integers, optional
    ...
    # if limb_color or colors are not provided, then paint everything in blue
    if limb_color is None:
        edge_colors = [[0, 0, 1.0] for _ in limb_color]
    else:
